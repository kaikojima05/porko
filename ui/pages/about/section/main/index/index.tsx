...
const AboutPage = forwardRef<HTMLDivElement>((_props, ref) => {
  return (
    <>
        ...
        <div className="before-scroll-repeat" ref={ref}>
          ...
          >
            ...
            >
              ...
              <div className={classNames("my-6", "md:my-0")}>
                {profile.map((prf, index) => (
                  <React.Fragment key={index}>
                    <p>{prf.content}</p>
                    <br
                      className={index + 1 === profile.length ? "hidden" : ""}
                    />
                  </React.Fragment>
                ))}
              </div>
              <div className="flex justify-end">
                ...
                >
                  <p className={classNames("text-base", "md:text-[1.125rem]")}>
                    sns -
                  </p>
                  <HoverIcons
                    iconsName={["sns-x", "sns-note"]}
                    hrefs={[
      ...
      <Section isTopMargin={false}>
        <HeadingH3 headingClassName="text-center">bio</HeadingH3>
        <div className={classNames("mt-5", "md:mt-6")}>
          {seriesData.map((item, index) => (
            <Supplement
              headLine={item.headLine}
              key={index}
              supplementClassName={classNames(
                `[&_br]:h-0 ${index !== 0 ? "mt-5" : "mt-0"}`
              )}
            ...
            >
              <p
                ...
                className={classNames("mt-2", "md:mt-0")}
              />
            </Supplement>
          ))}
        </div>
